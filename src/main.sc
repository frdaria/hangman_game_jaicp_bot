...
theme: /
    ...
    state: Hangman
        ...
        state: Start
            q: (нет/не надо/не нужно/сам помню/без правил/не сейчас/давай сразу) 
        ...
        state: NotStart
            q!: (не хочу/не буду/потом/не сейчас/нет/не надо/не нужно/хватит/стоп/достаточно)
            a: Ну и ладно...
            a: Если передумаешь, то пиши "игра"
            script:
                $response.replies = $response.replies || [];
                ...
                log("////// My LOG NotStart: " + toPrettyString($parseTree))
        ...
        state: Play
            ...
            state: CreateWord
                ...
                a: Я загадал это слово {{$session.hiddenWord}} ({{$session.originalWord}})
                a: Твой ход!
        ...
        # ПРОВЕРЯЕМ, не команда ли это
                        var fullInput = $request.query;
                        if (fullInput.startsWith("/")) {
                    # $reactions.answer("Напиши «Давай поиграем» чтобы начать игру!");
        ...
        # // --- ключевые подстроки для распознавания запроса помощи ---
                    ...
                    $session.isToStop = false;
                    for (var i = 0; i < stopKeywords.length; i++) {
                        if (input.indexOf(stopKeywords[i]) !== -1) {
                            $session.isToStop = true;
                            break;
                        }
                    }
            ...
            state: PlayerSayWord
                ...
                script:
                   if (!$session.guess || !$session.hiddenWord) {
                   //$reactions.answer("Игра еще не начата! Напиши «Давай поиграем»");
                   $reactions.transition("/Start")
                   return
